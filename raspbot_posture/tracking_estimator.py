...
def estimate_target_pose(
    ...
    centered_for_distance = abs(x_offset) <= config.distance_x_deadzone
    motion_allowed = (
        distance_m is not None
        and centered_for_distance
        and distance_confidence >= 0.35
        and state != "unknown"
    )
    ...
    if not centered_for_distance:
        reason = "target is not centered enough for distance control"
    elif state == "ok":
        reason = "distance is inside desired band"
    elif motion_allowed:
        reason = f"{model.name} estimates target is {state}"
    else:
        reason = "distance estimate confidence is too low"
...
class TargetTrackingInputBuilder:
    ...
    def from_features(
        ...
        return self.from_estimate(target)

    def from_estimate(self, target):
        ...
        return MotionTrackingInput(
            ...
            confidence=target.distance_confidence,
